...
function iniciaAssentos(seatmap/*, aviao*/) {
  let alfabeto = [
    "A",
    "B",
    "C",
  ...
  // let width /*seatsRow*/ = 10 //seatmap.data.decks[0].deckConfiguration.width //fesq + fdir + fmeio;
  let count = 0;
  //let isNull = false;
  let boolVar;
  // Alteracao 06.06.21
  let decks = [{}];
  //minLinhas = 6; // = seatmap["data"]["decks"][0]["deckConfiguration"][startseatRow];
  //maxLinhas = 10;// = seatmap.data.decks[0].deckConfiguration.endSeatRow;

  //width = 10 // == tamY
  //console.log(seatmap["data"][0]["decks"][0]["seats"]);
...
/*    let el2 = seatmap["data"][0]["decks"][0]["seats"][el];
    console.log(el2);*/
    let x = el.coordinates.x;
    let y = el.coordinates.y;
    count = y + x*width;

    //console.log((alfabeto[y].concat((parseInt(x,10)+1).toString())));
...
/*
  for(let i = minLinhas; i < maxLinhas + 1; i++){
    for (let j = 0; j < width){
      if(isNull){
        
      }
      seatNumber[i][j] = alfabeto[j].concat(i.toString())
      arrSeats.push(new assento(i+j, false, seatNumber));
    }
  }
  while (count < totalLinhas) {
    if (isNull) {
      ...
      let seatNumber = (count%seatsRow) + 1
